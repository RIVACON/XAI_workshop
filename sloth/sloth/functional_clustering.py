...
def _get_landmarks(f, landmark_type, rel_tol=0.001):
    ...
    if landmark_type == 'MIN':
        for i in range(f.shape[0]):
            result.append(get_local_minimum(f[i, :]))
    else:
        for i in range(f.shape[0]):
            result.append(get_local_maximum(f[i, :]))
    return result
...
def agglomerative(f,
    ...
    tmp = _get_landmarks(f_, 'MIN', rel_tol)
    ...
    return result, cluster_distance, f_[cluster_centers, :]
...
def fit_predict(f,
    ...
    method = "functional_clustering"
    param = UtilsClass.set_config(method)

    logger.info('Start functional clustering with n_clusters=' +
                str(param['n_clusters']) + ' and method ' + param['method'])
    # apply transformation to uniform [0,1] grid to make functions comparable
    if x_coords is not None:
        logger.debug(
            'Transform functions to uniform [0,1] grid width gridwidth ' + str(param['gridwidth']))
        f_transformed = _transform_standard_grid(x_coords, f, param['gridwidth'])
    else:
        f_transformed = f
    if param['method'] == 'euclidean':
        return kmeans(f_transformed, n_clusters=param['n_clusters'], random_state=param['random_state'], **param['sklearn_arg'])
    elif param['method'] == 'agglomerative':
        return agglomerative(f_transformed, n_clusters=param['n_clusters'], random_state=param['random_state'], rel_tol=param['rel_tol'], **param['sklearn_arg'])
    else:
        raise ValueError('Unknown method ' + param['method'])
